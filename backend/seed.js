...
async function seedDatabase() {
  ...
  try {
    ...
    // Delete existing meals with ID 1-20 to avoid conflicts
    await client.query('DELETE FROM meals WHERE id <= 20');
    console.log('✅ Cleared existing meals (1-20)');

    // Insert 20 pre-existing meals
    const insertQuery = `
    ...
    console.log('✅ Inserted 20 pre-existing meals');

    // Reset sequence
    await client.query(`SELECT setval('meals_id_seq', 20, true)`);
    console.log('✅ Reset ID sequence');

    // Verify
    const result = await client.query('SELECT COUNT(*) as count FROM meals');
    console.log(`\n🎉 Database seeded successfully!`);
    console.log(`📊 Total meals in database: ${result.rows[0].count}`);
    ...
    console.error('3. Ensure meal_planner database exists\n');
  } finally {
    client.release();
    await pool.end();
  }
